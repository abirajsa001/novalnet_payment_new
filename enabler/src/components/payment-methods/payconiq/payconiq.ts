...
export class PayconiqBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Payconiq(this.baseOptions, config);
  }
}
 
export class Payconiq extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.payconiq, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== PAYCONIQ ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Payconiq and transfer the shopping amount within the specified date.</p>
