...
export class WechatpayBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Wechatpay(this.baseOptions, config);
  }
}
 
export class Wechatpay extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.wechatpay, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== Wechatpay ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Wechatpay and transfer the shopping amount within the specified date.</p>
