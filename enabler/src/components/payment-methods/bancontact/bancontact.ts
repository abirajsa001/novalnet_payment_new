...
export class BancontactBuilder implements PaymentComponentBuilder {
  ...
  build(config: ComponentOptions): PaymentComponent {
    return new Bancontact(this.baseOptions, config);
  }
}
 
export class Bancontact extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.bancontact, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== BANCONTACT ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Bancontact and transfer the shopping amount within the specified date.</p>
