...
export class EpsBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Eps(this.baseOptions, config);
  }
}
 
export class Eps extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.eps, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== EPS ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Eps and transfer the shopping amount within the specified date.</p>
