...
export class TwintBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Twint(this.baseOptions, config);
  }
}
 
export class Twint extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.twint, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== Twint ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Twint and transfer the shopping amount within the specified date.</p>
