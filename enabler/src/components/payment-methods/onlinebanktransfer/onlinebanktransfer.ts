...
export class OnlinebanktransferBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Onlinebanktransfer(this.baseOptions, config);
  }
}
 
export class Onlinebanktransfer extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.onlinebanktransfer, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== OnlineBankTransfer ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with OnlineBankTransfer and transfer the shopping amount within the specified date.</p>
