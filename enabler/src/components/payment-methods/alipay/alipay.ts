...
export class AlipayBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Alipay(this.baseOptions, config);
  }
}
 
export class Alipay extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.alipay, baseOptions, componentOptions);
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Alipay and transfer the shopping amount within the specified date.</p>
