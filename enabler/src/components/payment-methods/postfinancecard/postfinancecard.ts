...
export class PostfinancecardBuilder implements PaymentComponentBuilder {
  public componentHasSubmit = true;
  constructor(private baseOptions: BaseOptions) {}

  build(config: ComponentOptions): PaymentComponent {
    return new Postfinancecard(this.baseOptions, config);
  }
}
 
export class Postfinancecard extends BaseComponent {
  private showPayButton: boolean;

  constructor(baseOptions: BaseOptions, componentOptions: ComponentOptions) {
    super(PaymentMethod.postfinancecard, baseOptions, componentOptions);
  ...
  async submit() {
    ...
    console.log('=== Postfinancecard ENABLER SUBMIT START ===');
  ...
  private _getTemplate() {
    ...
    <div class="${styles.wrapper}">
      <p>Pay easily with Postfinancecard and transfer the shopping amount within the specified date.</p>
