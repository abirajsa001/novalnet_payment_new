...
describe("mock-payment.service", () => {
  ...
  test("getSupportedPaymentComponents", async () => {
    ...
    expect(result?.components[4]?.type).toStrictEqual("onlinebanktransfer");
    expect(result?.components[5]?.type).toStrictEqual("sepa");
    expect(result?.components[6]?.type).toStrictEqual("creditcard");
